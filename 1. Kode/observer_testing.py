...
K = [0.567, 0.0045883] # very fast stabiliet
# K = [0.585, 0.00305] # new testing ground: K_k = 0.9, T_k = 2.54, K_1 = 0.65 * K_k, K_2 = 0.12 * T_k
# K = [0.7581, 0.0061] # kritisk stabilt

L = 5 # first try; I have no clue what im doing.
...
global counter # global variables.
x = [0.0]*10 # tom y[]
x_avg = [0.0]*10
y = [0.0]*10 # tom y[]
y_avg = [0.0]*10
...
def timerfunksjon():
    ...
    cycle_time = (time_array[0] - time_array[1])
    return cycle_time
...
def servo_control(key2, queue):
    ...
    def find_average(matrise, input, avg):
        ...
        for I in range(len(matrise)):
            tot += matrise[I]
        avg[counter] = (tot / len(matrise)) / 0.723
        counter += 1

    def find_speed(avg, fix):
        speed = (avg[counter - 1 - fix] - avg[counter - 3 - fix]) / (sample_time)
        return speed
    ...
    def PDreg(x_error, y_error):
        global servo_values, pos_y, pos_x, distance_error, K, L, counter, platform_angle, pre_platform_angle
